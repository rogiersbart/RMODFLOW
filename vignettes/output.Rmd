...
To save ASCII files, set the chedfm, cddnfm or cboufm values to proper FORTRAN formats enclosed in parenthesis containing no more than 20 characters. These FORTRAN formats can be found in the [MODFLOW-2005 manual](https://water.usgs.gov/ogw/modflow/MODFLOW-2005-Guide/index.html?oc.htm). Note that ASCII files are typically larger than binary files.
```{r}
rmf_create_oc(dis = dis, chedfm = "(10G11.4)") %>% 
  str()
```

Let's say, we want to save drawdowns in an ASCII file (setting cddnfm to a FORTRAN format) on unit number 50 (default is 667 for drawdown), not save heads, and print volumetric budget to the listing file every other time step:
```{r}
oc <- rmf_create_oc(dis = dis,
                    save_drawdown = TRUE, cddnfm = "(15F7.2)", iddnun = 50, 
                    save_head = FALSE, 
                    print_head = rep(c(FALSE, TRUE), sum(dis$nstp)/2))
str(oc)
```
