...
Lastly, in some models, not all boundary conditions are present in all layers. For example, wells are often present in deeper layers. Instead of plotting all layers separately to view them, we can reproject all boundary condition cells to the current layer when the `k` index is given by setting `to_k = TRUE`. Note that the IBOUND is not reprojected.
```{r, eval=F}
