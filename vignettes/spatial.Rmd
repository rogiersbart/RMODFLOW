...
```{r}
rmf_plot(dis$top, dis, prj = prj)
```

Using this approach, the `prj` object is part of the `dis` object and is by default directly obtained in function calls using `rmf_get_prj` (although all those functions allow specifying `prj` directly). As such, RMODFLOW projection information is written to the header comments of the DIS file when writing a `dis` object containing `prj` using `rmf_write_dis`. The structure of the RMODFLOW projection information is as follows:
...
Ultimately, these spatial-to-RMODFLOW functions might evolve to functions that are tailored for specific stress packages, e.g. a `rmf_build_wel` function that creates a `wel` object directly from a `sf` input object.
...
```{r}
dis <- rmf_create_dis(prj = rmf_create_prj(origin = c(152082, 168000.2), rotation = -12, crs = 31370))
rmf_has_prj(dis)

rmf_transform_prj(rmf_create_prj(origin = c(152082, 168000.2), crs = 31370), crs = 3812)
```
