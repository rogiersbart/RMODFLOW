...
### Subsetting
A `rmf_array` behaves like a common array in R with the two exceptions related to subsetting. 

1. Unlike R arrays, `drop = FALSE` by default for `rmf_arrays`. Subsetting a `rmf_array` will therefore not drop dimensions with length 1 or collapse into the lowest possible dimension unless the `drop = TRUE` argument is explicitly supplied.
...
ar <- array(dim = c(2,2,1,3))
ar[] <- rnorm(prod(dim(ar)))
dim(ar[,,,2])
...
### Time
There are two ways time is represented in MODFLOW: through time-steps and stress periods. Time-steps are inherit to the finite-difference equations used by MODFLOW whereas stress periods are used as a convenience for the user. A stress period encompases one or more time steps and all temporal input remains constant during the stress period. Arrays for input files such as recharge arrays can therefore vary per stress period. Output arrays such as simulated heads however, vary on a time-step basis. This creates a design choice for the `rmf_array`: does the 4th dimension represent time-steps or stress-periods? As stated above, the 4th dimension for a `rmf_array` represent time-steps. If you want a `rmf_array` to represent constant input over one or more stress periods, you can supply a `kper` argument to `rmf_create_array`:
...
To add to the complexity, fixed-format and free-format **EXTERNAL** and **OPEN/CLOSE** headers can point to binary files which contain the array. After the array-control records, the array is being read. Each control-record is explained [here](https://water.usgs.gov/ogw/modflow/MODFLOW-2005-Guide/index.html?drt.htm). RMODFLOW can read fixed-format and all 4 types of free-format array-headers. There is no support for writing fixed-format arrays.

To read MODFLOW arrays, RMODFLOW uses the internal `rmfi_parse_array` function which is called in all `rmf_read_*` functions that need to read in arrays. If the array-control record is fixed-format, a `nam` object (as returned from `rmf_read_nam`) needs to be supplied by the call to the `rmf_read_*` function. This is because the LOCAT variable represent a unit number on which to read the array. Without the corresponding NAME file, it is impossible to know from which file to read the array. The same is true when reading **EXTERNAL**  files. If the array is binary, the user must know if the array is single or double precision. By default, the `precision` argument is set to `'single'`.

Writing arrays to input files is handled by the internal `rmfi_write_array` function. By default, arrays are written in free-format using either the **CONSTANT** or **INTERNAL** array-control header, depending on whether or not there are more than 1 unique values in the supplied array. There is support for writing **EXTERNAL** and **OPEN/CLOSE** arrays to external files, but this is discouraged. Keeping all data in a single file creates a more clear and cohesive structure. To write **EXTERNAL**, you will need to supply the `external` argument to the `rmf_write_*` call which is a character vector with the names of the data sets to write. It will also need a `nam` object to check for a unique unit number (Nunit). You will have to add this unit number manually to a NAM file yourself. This is because the `rmfi_write_array` function is not intended to alter existing objects, only to write arrays. 
...
Both **EXTERNAL** and **OPEN/CLOSE** files can be written as binary by supplying a `binary` argument which is a character vector with the names of the data sets to write.
...
There is no separate `rmfi_write_list` function. Writing a list consist of looping over the rows in the list and calling `rmfi_write_variables` on the values in the row. There is currently no support for writing external or open/close lists. Writing of the SFAC value and of binary list is also not available. Because of the limited use of these capabilities, these functionalities might not be added at all in RMODFLOW.
