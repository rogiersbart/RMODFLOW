...
#'
#' \code{rmf_read_bal} reads a volumetric budget from a MODFLOW listing file and returns it as a list with data frame elements
#' 
#' @param file path to the listing file; typically '*.lst'
#'
#' @return an object of class bal which is a list with two data frames: one with cumulative fluxes and one with rates
#' @export
#' @importFrom readr read_lines

rmf_read_bal = function(file = {cat('Please select listing file ...\n'); file.choose()}){
...
#' @describeIn rmf_read_bal
#' @export
rmf_read_balance <- function(...) {
  rmf_read_bal(...)
}
