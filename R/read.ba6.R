...
read.ba6 <- function(file, dis=read.dis(paste(substring(file,1,nchar(file)-4),'.dis',sep='')))
{
  ...
  # Data set 1
    dataSet1 <- remove.empty.strings(strsplit(ba6.lines[1],' '))
    ba6 <- NULL
  ...
  # Data set 2
    dataSet2 <- get.mfarray(ba6.lines,ifelse(ba6$XSECTION,dis$NLAY,dis$NROW),dis$NCOL,ifelse(ba6$XSECTION,1,dis$NLAY))
  ...
  # Data set 4
    dataSet4 <- get.mfarray(ba6.lines,ifelse(ba6$XSECTION,dis$NLAY,dis$NROW),dis$NCOL,ifelse(ba6$XSECTION,1,dis$NLAY))
    ba6.lines <- dataSet4$remaining.lines
    ba6$STRT <- dataSet4$mfarray
    rm(dataSet4)
  
  class(ba6) <- c('ba6','mfpackage')
  return(ba6)
}
