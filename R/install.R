...
rmfi_install_code <- function(
  ...
  if (any(grepl("2005", code))) {
    ...
    rmfi_download_code("MODFLOW-2005", file_url, version, path, folder)
  }
  if (any(grepl("OWHM", code, ignore.case = TRUE))) {
    ...
    rmfi_download_code("MODFLOW-OWHM", file_url, version, path, folder)
  }
  if (any(grepl("NWT", code, ignore.case = TRUE))) {
    ...
    rmfi_download_code("MODFLOW-NWT", file_url, version, path, folder)
  }
  if (any(grepl("LGR", code, ignore.case = TRUE))) {
    ...
    rmfi_download_code("MODFLOW-LGR", file_url, version, path, folder)
  }
  if (any(grepl("CFP", code, ignore.case = TRUE))) {
    ...
    rmfi_download_code("MODFLOW-CFP", file_url, version, path, "MODFLOW-CFP")
  }
}
rmfi_download_code <- function(
  ...
  if (!fs::dir_exists(fs::path(path, code_name))) {
    temp <- tempfile()
    message("Downloading file ...")
    download.file(file_url, temp, quiet = TRUE)
    message("Installing ...")
    if (code_name == "MODFLOW-CFP") {
      unzip(temp, exdir = fs::path(path, code_name))
    } else {
      unzip(temp, exdir = path)
      fs::file_move(fs::path(path, folder), fs::path(path, code_name))
    }
    unlink(temp, force = TRUE)
    cat(paste0(version, "\n"), file = fs::path(path, code_name, "version-number-for-RMODFLOW"))
    message("You have succesfully installed ", code_name, " (v",
            version, ") at:\n", fs::path(path, code_name))
  } else {
    installed_version <- readLines(fs::path(path, code_name, "version-number-for-RMODFLOW"))
    ...
    if (install == "y") {
      fs::file_delete(fs::path(path, code_name))
