...
read.hob <- function(file)
{
  ...
  # Data set 0
    hob.lines <- remove.comments.from.lines(hob.lines)
  
  # Data set 1
    line.split <- split.line.char(hob.lines[1]); hob.lines <- hob.lines[-1]
  ...
  # Data set 2
    line.split <- split.line.num(hob.lines[1]); hob.lines <- hob.lines[-1]
