...
#' @export
int_get_modflow_array <- function(mfarray.lines,NROW,NCOL,NLAY)
{
  ...
  # Read array according to format type if there is anything to be read
    if(prod(dim(mfarray))!=0)
    {
      ...
      { 
        ...
        if(remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[1]=='CONSTANT') 
        {
          if(NLAY==1)
          {
            mfarray <- as.numeric(remove_empty_strings(strsplit(mfarray.lines[1],' |\t')[[1]])[2])
        ...
        else if(remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[1]=='INTERNAL')
        {
          mfarray.lines <- mfarray.lines[-1] 
        ...
        else if(remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[1]=='OPEN/CLOSE')
        {
          ...
          mfarray[,,k] <- as.matrix(read.table(file=remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[2]))
          mfarray.lines <- mfarray.lines[-1] 
        }   
      }
    }
  ...
  # Set class of object (2darray; 3darray)
    if(NLAY==1){mfarray <- as.matrix(mfarray[,,1]); class(mfarray) <- 'modflow_2d_array'}
    if(NLAY!=1) class(mfarray) <- 'modflow_3d_array'
