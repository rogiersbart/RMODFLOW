...
read_pcg <- function(file = {cat('Please select pcg file ...\n'); file.choose()}) {
  
  pcg_lines <- read_lines(file)
  pcg <- list()
  
  # data set 0
    data_set_0 <- read_modflow_comments(pcg_lines)
    comment(pcg) <- data_set_0$comments
    pcg_lines <- data_set_0$remaining_lines
    rm(data_set_0)
    
  # data set 1
    data_set_1 <- read_modflow_variables(pcg_lines)
  ...
  # data set 2
    data_set_2 <- read_modflow_variables(pcg_lines)
  ...
  class(pcg) <- c('pcg','modflow_package')
  return(pcg)
}
