...
rmfi_get_balance=function(balance_lines){ # balance lines is a character vector of length n where each element is a line of a text file with n lines. The first line should be "VOLUMETRIC BUDGET OF THE ENTIRE MODEL AT TIME STEP K, STRESS PERIOD M"; the last line should be "PERCENT DISCREPANCY = ..."
  ...
  header_lines = remove_empty_strings(strsplit(header, ' ')[[1]])
  ...
  for (i in 1:(length(balance_lines_inflow)-2)){
    ...
    tmp_in = remove_empty_strings(strsplit(balance_lines_inflow[i], ' ')[[1]])
    tmp_out = remove_empty_strings(strsplit(balance_lines_outflow[i], ' ')[[1]])
  ...
  difference_lines_split =  remove_empty_strings(strsplit(difference_lines, ' ')[[1]])
  ...
  perc_discrepancy_lines_split = remove_empty_strings(strsplit(perc_discrepancy_lines, ' ')[[1]])
