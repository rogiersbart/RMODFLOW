...
write_hob <- function(hob,
  ...
  # data set 3 - 6
    ...
    while(i <= length(hob$layer)) {
      write_modflow_variables(hob$obsnam[i], hob$layer[i], hob$row[i], hob$column[i], hob$irefsp[i], hob$toffset[i][1],hob$roff[i], hob$coff[i], hob$hobs[i],file=file) 
      if(hob$layer[i] < 0) {
      ...
      if(hob$irefsp[i] < 0) {
        ...
        # data set 6
          for(j in 1:abs(hob$irefsp[i])) {
            i <- i + 1
            write_modflow_variables(hob$obsnam[i], hob$irefsp[i], hob$toffset[i],hob$hobs[i], file=file)
          }
      }
      i <- i + 1
    }
