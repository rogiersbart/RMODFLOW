...
write_pcg <- function(pcg,
  ...
  # data set 1
    write_modflow_variables(pcg$mxiter, pcg$iter1, pcg$npcond, ifelse(is.na(pcg$ihcofadd),'',pcg$ihcofadd),file=file)
 
  # data set 2
    write_modflow_variables(pcg$hclose, pcg$rclose, pcg$relax, pcg$nbpol, pcg$iprpcg, pcg$mutpcg, pcg$damppcg, ifelse(is.na(pcg$damppcgt),'',pcg$damppcgt), file = file)
}
