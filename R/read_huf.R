...
read_huf <- function(file = {cat('Please select huf file ...\n'); file.choose()},
  ...
  # data set 1
    data_set_1 <- read_modflow_variables(huf_lines)
  ...
  # data set 2
    data_set_2 <- read_modflow_variables(huf_lines)
    huf$lthuf <- data_set_2$variables
    huf_lines <- data_set_2$remaining_lines
    rm(data_set_2)
  
  # data set 3   
    data_set_3 <- read_modflow_variables(huf_lines)
  ...
  # data set 4
    if(sum(huf$laywt > 0)) {
      data_set_4 <- read_modflow_variables(huf_lines)
  ...
  # data set 6-8
    ...
    for(i in 1:huf$nhuf) {
      data_set <- read_modflow_variables(huf_lines)
  ...
  # data set 9
    ...
    data_set_9 <- read_modflow_variables(huf_lines)
  ...
  # data set 10-11
    ...
    for(i in 1:huf$nphuf) {
      data_set_10 <- read_modflow_variables(huf_lines)
      ...
      for(j in 1:huf$nclu[i]) {
        data_set_11 <- read_modflow_variables(huf_lines)
