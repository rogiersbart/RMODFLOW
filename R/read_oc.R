...
read_oc <- function(file = {cat('Please select oc file ...\n'); file.choose()}) {

  oc_lines <- read_lines(file)
  oc <- list()
  
  # data set 0
    data_set_0 <- read_modflow_comments(oc_lines)
  ...
  # data set 1
    ...
    while(read_modflow_variables(oc_lines[1])$variables[1] != 'PERIOD') {
      data_set_1 <- read_modflow_variables(oc_lines[1])$variables
  ...
  # data set 2 & 3
    ...
    while(length(oc_lines) != 0) {
      data_set_2 <- read_modflow_variables(oc_lines[1])$variables
      oc_lines <- oc_lines[-1]
      oc$iperoc <- append(oc$iperoc, as.numeric(data_set_2[2]))
      oc$itsoc <- append(oc$itsoc, as.numeric(data_set_2[4]))
      while(read_modflow_variables(oc_lines[1])$variables[1] != 'PERIOD' & length(oc_lines) != 0) {
