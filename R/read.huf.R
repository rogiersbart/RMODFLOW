...
read.huf <- function(file, dis=read.dis(paste(substring(file,1,nchar(file)-4),'.dis',sep='')))
{
  ...
  # Data set 1
    dataSet1 <- split.line.num(huf.lines[1])
  ...
  # Data set 4
    if(sum(huf$LAYWT > 0))
    {
      dataSet4 <- split.line.num(huf.lines[1])
      huf.lines <- huf.lines[-1]
  ...
  # Data set 5
    dataSet5 <- get.mfarray(huf.lines,dis$NROW,dis$NCOL,sum(which(huf$LAYWT!=0)))
  ...
  # Data set 6-8
    ...
    for(i in 1:huf$NHUF)
    {
      ...
      huf.lines <- huf.lines[-1]
      dataSet <- get.mfarray(huf.lines,dis$NROW,dis$NCOL,2)
