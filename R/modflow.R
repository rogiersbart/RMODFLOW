...
#'
#' @param ... \code{RMODFLOW} objects of class \code{rmf_package} to be included in the modflow object. If a nam object is not provided, it is added automatically.
...
rmf_write <- function(modflow, 
  ...
  # mlt & zon
  if('mlt' %in% ftype) rmf_write_mlt(mlt = modflow$mlt, file = paste(dir_name, modflow$nam$fname[which(modflow$nam$ftype == 'MULT')], sep = '/'))
  if('zon' %in% ftype) rmf_write_zon(zon = modflow$zon, file = paste(dir_name, modflow$nam$fname[which(modflow$nam$ftype == 'ZONE')], sep = '/'))
  
  # pvl
  if('pvl' %in% ftype) rmf_write_pvl(pvl = modflow$pvl, file = paste(dir_name, modflow$nam$fname[which(modflow$nam$ftype == 'PVAL')], sep = '/'))
    
  # lgr
  # if('lgr' %in% ftype) rmf_write_lgr(lgr = modflow$lgr, file = paste(dir_name, modflow$nam$fname[which(modflow$nam$ftype == 'LGR')], sep = '/'))
